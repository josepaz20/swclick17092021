...
//    $("#passwordver").val($('#passwordpppoe_' + idServicio).text());
    $("#passwordSinFormato").html($('#passwordpppoe_' + idServicio).text());    
    $("#modalCredenciales").modal('show');    
}


//---------------------------------------------------------------------------------------------
//Cortar Servicios Segmento
function verCortarServiciosSegmento(idSegmento, segmento) {
    $.get('vercortarserviciossegmento', {idSegmento: idSegmento, segmento: segmento}, setFormulario, 'json');
    activarBloqueoAjax();
}
//Suspender Servicios Segmento
function verSuspenderServiciosSegmento(idSegmento, segmento) {
    $.get('versuspenderserviciossegmento', {idSegmento: idSegmento, segmento: segmento}, setFormulario, 'json');
    activarBloqueoAjax();
}


function verActualizarQueuesSegmento(idSegmento, segmento) {
    $.get('veractualizarqueuessegmento', {idSegmento: idSegmento, segmento: segmento}, setFormulario, 'json');
    activarBloqueoAjax();
}


function quitarPPPoE(idServicio) {
    if (confirm("  DESEA QUITAR EL PPPoE DE ESTE SERVICIO ?")) {
        $.get('quitarpppoe', {idServicio: idServicio}, setQuitarpppoe, 'json');
        activarBloqueoAjax();
    }
}
function setQuitarpppoe(datos) {
    document.location.href = '/sw2click/modulos/mikrotik/gestion?msg=' + datos;
}
