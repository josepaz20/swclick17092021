...
function verAsignacionIpPublica(idServicio) {
    $.get('asignacionIpPublica', {idServicio: idServicio}, setFormulario, 'json');
    activarBloqueoAjax();
}
...
function verificarIP(tipoIP) {
    ...
    if ($("#mascara").length !== 0) {
        if (parseInt($("#mascara").val()) >= 8 && parseInt($("#mascara").val()) <= 32) {
            mascara = $("#mascara").val();
        } else {
            alert(" LA MASCARA: " + ip + " \n ESTA FUERA DEL RANGO PERMITIDO, DEBE ESTAR ENTRE 8 Y 32. ");
            $("#mascara").focus();
            return;
        }
    }
    ...
    if (ip !== '') {
        if (ip.match(/((^|\.)((25[0-5])|(2[0-4]\d)|(1\d\d)|([1-9]?\d))){4}$/)) {
            ...
            if (partesIP.length === 4) {
                if (tipoIP === 'publica') {
                    if (parseInt(partesIP[0]) !== 138 && parseInt(partesIP[1]) !== 0) {
                        alert(" LA DIRECCION IP PUBLICA: " + ip + " \n ESTA FUERA DEL RANGO DE IPs PUBLICAS PERMITIDAS. ");
                        ipInput.focus();
                        return;
                    }
                }
                if (parseInt(partesIP[3]) > 2 && parseInt(partesIP[3]) !== 255) {
                    $.get('verificarIP', {dirIP: ip, tipoIP: tipoIP, mascara: mascara}, setVerificarIP, 'json');
                    activarBloqueoAjax();
                } else {
...
function setVerificarIP(datos) {
    if (parseInt(datos['error']) === 0) {
        var iconBAD = 'iconBAD';
        var iconOK = 'iconOK';
        if (datos['tipoIP'] === 'publica') {
            iconBAD = 'iconBADpublica';
            iconOK = 'iconOKpublica';
        }
...
function validarAsignacionIPpublica() {
    return confirm(" � DESEA ASIGNAR LA IP PUBLICA A ESTE SERVICIO ? ");
}
