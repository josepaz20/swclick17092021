...
//------------------------------------------------------------------------------
var idAntenaTXSubmit = 0;
var idSC = 0;
function buscarServicioAntena(idAntenaTX) {
    idAntenaTXSubmit = idAntenaTX;
    swal({
        title: 'Ingresa el <u>ID</u> que deseas buscar',
        input: 'number',
        type: 'info',
        inputAttributes: {
            autocapitalize: 'off'
        ...
        padding: '4em',
        background: '#fff',
        footer: '<a href="/sw2click/modulos/infraestructura/antenas">Refresacar pagina</a>',
...
function buscarIdSerCont() {
    swal({
        title: 'Donde deseas <u>bucar</u> con este <u>ID ' + idSC + '</u>',
        type: 'info',
        allowEscapeKey: false,
        focusConfirm: false,
        showCloseButton: true,
        ...
        confirmButtonColor: '#0489B1',
        cancelButtonColor: '#088A68',
        confirmButtonText:
                '<i class="fa fa-search"></i> Buscar Contrato',
        cancelButtonText:
                '<i class="fa fa-search"></i> Buscar Servicio',
        width: 600,
        padding: '4em',
        background: '#fff',
        footer: '<a href="/sw2click/modulos/infraestructura/antenas">Refresacar pagina</a>',
        ...
        if (result.value) {
            $.get('serviciosContrato', {idServicio: 0, idContrato: idSC, idAntenaTX: idAntenaTXSubmit}, setFormularioServiciosContrato, 'json'), activarBloqueoAjax();
            swal.close();
        } else {
            $.get('serviciosContrato', {idServicio: idSC, idContrato: 0, idAntenaTX: idAntenaTXSubmit}, setFormularioSC, 'json'), activarBloqueoAjax();
            swal.close();
        }
    });
}

function setFormularioSC(respuestaServidor) {
    if (parseInt(respuestaServidor['error']) === 2) {
        swal({
            title: 'Imposible conseguir datos para este Servicio <u>' + idSC + '</u>',
            type: 'error',
            allowEscapeKey: false,
            focusConfirm: false,
    ...
    if (parseInt(respuestaServidor['error']) === 0) {
        swal({
            title: 'SERVICIO <u>ID ' + idSC + '</u>',
            type: 'success',
            html: "<br>\n\
                   <form id='alert-update-antena' method='POST'>\n\
                        Id Servicio:\n\
                        <input value='" + respuestaServidor['idServicio'] + "' name='idsServicio' class='swal2-input' type='text' readonly><br>\n\
                        Cliente: \n\
                        <input value='" + respuestaServidor['cliente'] + "' class='swal2-input' type='text' readonly>\n\
                        Servicio: \n\
                        <input value='" + respuestaServidor['servicio'] + "' class='swal2-input' type='text' readonly>\n\
                        Estado: \n\
                        <input value='" + respuestaServidor['estado'] + "' class='swal2-input' type='text' readonly>\n\
                        <input value='" + respuestaServidor['idAntenaTX'] + "' name='idAntenaTX' class='swal2-input' type='hidden'>\n\
                   <form>\n\
                ",
            allowEscapeKey: false,
            focusConfirm: false,
            ...
            padding: '4em',
            background: '#fff',
            footer: '<p>Busqueda Exitosa</p>',
            showLoaderOnConfirm: true
        }).then(function (result) {
            if (result.value) {
                $("#alert-update-antena").attr('action', 'registrarServicioAntena');
                $("#alert-update-antena").submit();
                activarBloqueoAjax();
                swal.close();
            }
        });
...
function setFormularioServiciosContrato(respuestaServidor) {
    if (parseInt(respuestaServidor['error']) === 2) {
        swal({
            title: 'Imposible conseguir datos para este Contrato <u>' + idSC + '</u>',
            type: 'error',
            allowEscapeKey: false,
            focusConfirm: false,
            showCloseButton: false,
