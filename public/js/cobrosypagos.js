...
function verHistorialEnvioProforma(idProforma) {
    $.get('verHistorialEnvio', {idProforma: idProforma}, setFormulario);
    bloqueoAjax();
}
...
//------------------------------------------------------------------------------
function verRegistrarCobroProforma(tipo) {
    if (tipo === 'corporativo') {
        $.get('verRegProformaCorp', {}, setRegistrarCobroProforma);
    } else {
...
function setNuevoConceptoCorp() {
    ...
    newfila += '<option value="INSTALACION_3">INSTALACION</option>';
    newfila += '<option value="EQUIPO_6">EQUIPO</option>';
    newfila += '<option value="CLAUSULA PERMANENCIA_8">CLAUSULA PERMANENCIA</option>';
    newfila += '</select>';
    newfila += '</td>';
    newfila += '<td>';
...
function getServicioInfoCorp(idServicio) {
    if (idServicio !== '') {
        $.get('getServicioInfo', {idServicio: idServicio}, setServicioInfoCorp, 'json');
    } else {
        $("#idServicio").val(0);
        $("#DIVINFOSERVICIOHIDDEN").hide('slow');
    }
}
function setServicioInfoCorp(respuesta) {
    if (parseInt(respuesta['existe']) === 1) {
        $("#DIVINFOSERVICIOHIDDEN").show('slow');
        $("#idServicio").val(respuesta['idServicio']);
        $("#conceptoServicio").val(respuesta['conceptoFacturacion']);
        $("#vlrBase").val(respuesta['cargoFijoMes']);
        $("#vlrServicio").val(respuesta['totalPago']);
        $("#estado").val(respuesta['estado']);
        var numDecos = respuesta['numDecos'];
        if (numDecos === '' || numDecos === 'null') {
...
function agregarServicio() {
    if ($("#servicioSelect").val() !== '') {
        $("#numConceptos").val(parseInt($("#numConceptos").val()) + 1);
        var i = parseInt($("#numConceptos").val());
        var vlrbase = parseInt($("#vlrBase").val());
        var total = 0; //Math.round(vlrbase * 1.19);
        var iva = 0; //total - vlrbase;
        var newfila = '<tr>';
        newfila += '<td>';
        newfila += '<input type="number" name="idServicio_' + i + '" id="idServicio_' + i + '" value="' + $("#servicioSelect").val() + '" style="width: 80px !important" class="form-control" readonly>';
        newfila += '</td>';
        newfila += '<td>';
        newfila += '<select id="tipoCobro_' + i + '" name="tipoCobro_' + i + '" class="form-control" style="width: 130px" onchange="conceptoCobro(this.value, this.id)">';
        newfila += '<option value="1">SERVICIO INTERNET</option>';
        newfila += '<option value="2">SERVICIO TV</option>';
        newfila += '<option value="3">INSTALACION</option>';
        newfila += '<option value="6">EQUIPO</option>';
        newfila += '<option value="8">CLAUSULA PERMANENCIA</option>';
        newfila += '</select>';
        newfila += '</td>';
        newfila += '<td>';
        newfila += '<input class="form-control" name="concepto_' + i + '" id="concepto_' + i + '" value="' + $("#conceptoServicio").val() + '">';
        newfila += '</td>';
        newfila += '<td>';
        newfila += '<input class="form-control" value="' + vlrbase + '" type="number" name="vlrbase_' + i + '" id="vlrbase_' + i + '" min="0" onchange="calcularTotales(' + i + ')" style="width: 100px" required>';
        newfila += '</td>';
        newfila += '<td>';
        newfila += '<input class="form-control" value="' + iva + '" type="number" name="vlriva_' + i + '" id="vlriva_' + i + '" min="0" onchange="calcularTotales(' + i + ')" style="width: 100px" readonly>';
        newfila += '<input type="checkbox" id="aplicaiva_' + i + '" name="aplicaiva_' + i + '" onclick="calcularTotales(' + i + ')" checked> <label for="aplicaiva_' + i + '">Aplica IVA</label>';
        newfila += '</td>';
        newfila += '<td>';
        newfila += '<input class="form-control" value="' + total + '" type="number" name="vlrtotal_' + i + '" id="vlrtotal_' + i + '" min="1" onchange="calcularTotales(' + i + ')" style="width: 100px" readonly>';
        newfila += '</td>';
        newfila += '</tr>';
        $("#tablaCobros").append(newfila);
        calcularTotales(i);
...
function agregarServicioEdit() {
    if ($("#servicioSelect").val() !== '') {
        ...
        newfila += '</td>';
        newfila += '<td>';
//        newfila += '<a href=""><i class="fa fa-times"></i></a>';
        newfila += '<i style="color: blue">NUEVO</i>';
        newfila += '</td>';
        newfila += '<td>';
        newfila += '<select id="tipoCobro_' + i + '" name="tipoCobro_' + i + '" class="form-control" style="width: 130px" onchange="conceptoCobro(this.value, this.id)">';
        newfila += '<option value="1">SERVICIO INTERNET</option>';
        ...
        newfila += '<td>';
        newfila += '<input class="form-control" value="' + iva + '" type="number" name="vlriva_' + i + '" id="vlriva_' + i + '" min="0" onchange="calcularTotales(' + i + ')" style="width: 100px" readonly>';
        newfila += '<input checked type="checkbox" id="aplicaiva_' + i + '" name="aplicaiva_' + i + '" onclick="calcularTotales(' + i + ')"> <label for="aplicaiva_' + i + '">Aplica IVA</label>';
        newfila += '</td>';
        newfila += '<td>';
        newfila += '<input class="form-control" value="' + total + '" type="number" name="vlrtotal_' + i + '" id="vlrtotal_' + i + '" min="1" onchange="calcularTotales(' + i + ')" style="width: 100px" readonly>';
        newfila += '</td>';
...
function calcularTotales(idCobro) {
    var vlrbase = $("#vlrbase_" + idCobro).val();
    if (vlrbase === '' || parseInt(vlrbase) < 0) {
        $("#vlrbase_" + idCobro).val(0);
        alert(" EL VALOR BASE DEL COBRO NO PUEDE SER MENOR, VACIO O IGUAL A CERO !");
        $("#vlrbase_" + idCobro).focus();
        $("#vlr-total-proforma").val("$ 0");
        return false;
    }
    vlrbase = parseInt(vlrbase);
    if (vlrbase > 0) {
        if ($("#aplicaiva_" + idCobro).is(':checked')) {
            var total = Math.round(vlrbase * 1.19);
            var iva = total - vlrbase;
            $("#vlriva_" + idCobro).val(iva);
            $("#vlrtotal_" + idCobro).val(total);
        } else {
            $("#vlriva_" + idCobro).val(0);
            $("#vlrtotal_" + idCobro).val(vlrbase);
        }
    }
    calcularVlrTotalProforma();
}
//------------------------------------------------------------------------------
function verEditProformaCorp(idProforma) {
    $.get('verEditProformaCorp', {idProforma: idProforma}, setFormularioProforma);
}
//------------------------------------------------------------------------------
function validarConceptosAgregados() {
    if (parseInt($("#numConceptos").val()) > 0) {
        if (confirm(" DESEA GUARDAR LOS COBROS AGREGADOS ?")) {
            bloqueoAjax();
            return true;
        } else {
            return false;
        ...
        alert("NO SE HAN AGREGADO NUEVOS COBROS");
        return false;
    }
}
//------------------------------------------------------------------------------
function eliminarConceptoEdit() {
    if (parseInt($("#numConceptos").val()) > 0) {
        if (confirm(" DESEA ELIMINAR EL ULTIMO COBRO AGREGADO ?")) {
            var tabla = document.getElementById('tablaCobros');
            var ultFila = tabla.rows.length;
            tabla.deleteRow(ultFila - 1);
            $("#numConceptos").val(parseInt($("#numConceptos").val()) - 1);
        ...
        alert("NO SE HAN AGREGADO NUEVOS COBROS, NO ES POSIBLE ELIMINAR");
    }
}
...
function validarNuevaProforma() {
    ...
    if (confirm(' DESEA GENERAR ESTA PROFORMA ?')) {
        bloqueoAjax();
        return true;
    } else {
        return false;
