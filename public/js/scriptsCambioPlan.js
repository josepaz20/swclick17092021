...
function setInfoPlan(datos) {
    $("#concepto").val('');
    if (parseInt(datos['error']) === 0) {
        var totalPago = parseFloat(datos['totalPago'].replace(/\,/g, ''));
        var pagoMensual = parseFloat($("#pagoMensual").val().replace(/\,/g, ''));
        if (totalPago >= pagoMensual) {
            $("#divInfoPlan").html(datos['html']);
            $("#idPlan").val(datos['idPlan']);
            $("#concepto").focus();
        } else {
            alert("NO ES POSIBLE SELECCIONAR ESTE PLAN, EL VALOR TOTAL DEL PLAN A SELECCIONAR DEBE SER MAYOR O IGUAL AL PLAN ACTUAL DEL SERVICIO");
            return false;
        }
    } else {
        alert(" SE HA PRESENTADO UN ERROR EN EL SISTEMA. \n\n POR FAVOR COMUNIQUESE CON EL ADMINISTRADOR.");
...
function rechazarCambioPlan(idCambioPlan) {
    if (confirm("¿ DESEA RECHAZAR ESTE CAMBIO DE PLAN ?")) {
        $.get('rechazarcambioplan', {idCambioPlan: idCambioPlan}, setRechazarCambioPlan, 'json');
        bloqueoAjax();
    }
}
function setRechazarCambioPlan(datos) {
    if (parseInt(datos) === 0) {
        alert("EL CAMBIO DE PLAN HA SIDO RECHAZADO EN JOSANDRO");
        location.reload();
    } else {
        alert("ERROR, EL CAMBIO DE PLAN NO PUDO SER RECHAZADO.\nPOR FAVOR INTENTE DE NUEVO O COMUNIQUESE CON EL ADMINISTRADOR DEL SISTEMA.");
        return false;
    }
}