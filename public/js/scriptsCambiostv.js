...
function setTvAnaloga() {
    if ($("#tvanaloga").prop('checked')) {
        $("#numDecos").val(0);
        $("#totalDecos").val(0);
        $("#numDecos").attr('readonly', true);
    } else {
        $("#numDecos").val('');
        if ($("#numDecosOLD").length > 0) {
            $("#totalDecos").val($("#numDecosOLD").val());
        } else {
            $("#totalDecos").val('');
        }
        $("#numDecos").removeAttr('readonly');
    }
}

function validarEliminacion() {
    if (confirm("   DESEA ELMINAR ESTA SOLICITUD ? ")) {
        activarBloqueoAjax();
        return true;
    } else {
        return false;
